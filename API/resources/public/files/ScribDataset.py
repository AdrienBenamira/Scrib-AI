...
from utils import get_vector_article, pad_sequence, tokenize
...
class ScribDataset(Dataset):
    ...
    def __init__(self, scribAPI, word_to_index=None, index_to_word=None, name="cnn-daily", transform=None, pad_token='<pad>'):
    ...
    def get_batch(self, batch_size, start=None, generator=False):
        ...
        data = self.__getitem__(slice(start, start + batch_size))
        articles, summaries = data['article'], data['summaries']
        articles = list(map(lambda x: " ".join(tokenize(x)), articles))
        summaries = list(map(lambda x: x[0], summaries))
        summaries = list(map(lambda x: " ".join(tokenize(x)), summaries))
        # articles_data = self.__getitem__(slice(start, start + batch_size))['article']
        # length_articles = [len(article) for article in articles]
        # length_summaries = [len(summary) for summary in summaries]
